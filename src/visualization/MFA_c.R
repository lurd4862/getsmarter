MFA_c = function (base, group, type = rep("s", length(group)), excl = NULL, 
                                                                      ...
                                                                      2), tab.comp = NULL) 
{
  ...
  funcLg <- function(x, y, ponderation.x, ponderation.y, wt = rep(1/nrow(x), 
    ...
    wt <- wt/s
    center <- colSums(wt * x)
    x <- sqrt(wt) * t(t(sweep(x, 2, center, check.margin = FALSE)) * 
                        sqrt(ponderation.x))
    center <- colSums(wt * y)
    y <- sqrt(wt) * t(t(sweep(y, 2, center, check.margin = FALSE)) * 
                        sqrt(ponderation.y))
    Lg <- 0
  ...
  base[, is.quali] <- lapply(base[, is.quali, drop = FALSE], 
                             as.factor)
  ...
  nom.ligne <- c(sapply(rownames(base)[1:nb.actif], paste, 
  ...
  inertie.intra.ind.partiel <- data.frame(do.call(rbind, lapply(1:dim(tmp)[1], 
                                                                function(x) t(tmp[x, 1:ncp, ]))))
  ...
  eig <- res.globale$eig
  nom.ligne <- c(sapply(rownames(base), paste, name.group[group.actif], 
                        sep = "."))
